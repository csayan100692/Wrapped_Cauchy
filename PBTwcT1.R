...
PBTwcT1=function(x,alpha)
{
  ...
  #################################################################
      ...
      d1=min(bar_nu-nu_0,2*pi-(bar_nu-nu_0))
      ...
      foreach(h = 1:H) %do%
        {
          ...
          d11[h]=min(bar1_nu[h]-nu_0,2*pi-(bar1_nu[h]-nu_0))
  ...
  return(cat("test statistic and critical value for tm is = ", c(statistic_value, critical_value),"\n"))
}
