...
PBTWRPCAUCHYCP1=function(x,alpha)
{
  library(circular)
  library(CircStats)
  library(doParallel)
  ...
  ####################################################################
      ...
      CI_s1<-(quantile.circular(bar1_nu,probs = c(alpha/2,1-alpha/2)))
      ...
      L1<-mean(bar1_nu)-min(as.numeric(CI_s1))
      ...
      R1<-max(as.numeric(CI_s1))-mean(bar1_nu)
      ...
      # agldata[3,]<-as.numeric(c(I2[1],I2[2],y,nu_0))
  ...
  return(cat("COVERGE PROBABILITY and Length For tm is =",c(covg1,l1),"\n"))
}
